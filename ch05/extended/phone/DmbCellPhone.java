package ch05.extended.phone;

public class DmbCellPhone extends CellPhone{ // 자식 라이브러리 클래스
	// 2025.05.28(수)
	// 상속 Inheritance
	// 자식 클래스 : 부모필드 메서드 상속 및 추가필드 메서드 제공
	
	// 필드
	int channel ; // 자식 필드 추가
				  // dmb폰은 방송을 볼수있는 기능이 추가됨.
	
	// 생성자  -> 매개값을 입력받아 초기값을 진행
	DmbCellPhone (String model, String color, int channel) {
		//super(매개값); 매개값의 타입과 일치하는 부모생성자를 호출함.
		//super(); 생략시, 컴파일러에 의해 super()가 자동적으로 추가됨 
		//        					   -> 부모의 기본 생성자가 존재해야함.
		this.model = model ;   // 부모필드
		this.color = color ;   // 부모필드
		this.channel = channel ;  // 자식필드
		
	}
	
	
	// 메서드  -> 자식 클래스에 추가된 기능
	void turnOnDmb() {
		System.out.println("채널" + channel + "번 DMB 방송 수신을 시작합니다.");
	} // DMB 방송시작
	
	void changeChannelDmb(int channel) {
		this.channel = channel ; // 채널번호를 입력받아 채널 변경을 진행함. 
								 // 입력값은 자식필드 channel 변수에 들어감.
		System.out.println("채널을" + channel +"번으로 변경합니다.");
	} // DMB채널변경
	
	void turnOffDmb () {
		System.out.println("DMB 방송 수신을 중지합니다.");
	} // DMB 방송종료
	
	
	
	
	
}
